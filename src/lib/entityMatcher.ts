import {
    ...
    Region,
    Country
} from "../utils/preprocessHelpers"
import { CityPopulation } from "./populations"
...
export default function(
    ...
    if (extractedEntities[extractedEntities.length - 1].length == 0) {
    ...
    if (finalCandidates.length == 1) return locationToString(finalCandidates[0])

    const country = finalCandidates.filter(
        candidate => (candidate as Country).iso2
    )
    if (country.length > 0) return locationToString(country[0])
    ...
    return locationToString(candidatesWithPop[0])
}
